...
var (
	...
	// StartTLSServerFuncs maps a StartTLS flavor name to it StartTLS handshake handler.
	StartTLSServerFuncs = map[string]func(WriteCloser) (WriteCloser, error){
		"postgres": HandlePostgresStartTLSHandshakeAsServer,
	}

	// StartTLSClientFuncs maps a StartTLS flavor name to it StartTLS handshake handler.
...
// handshake (acting as client). It sends the postgresStartTLSMsg
// and checks if the server response matches the expected value.
...
func HandlePostgresStartTLSHandshakeAsClient(conn io.ReadWriter) error {
	...
	if b[0] != PostgresStartTLSReply[0] {
		...
		// and cannot forward.
		return fmt.Errorf("postgres does not accept tls. response got %v want %s ", b, PostgresStartTLSReply[0])
	}
	return nil
}

// HandlePostgresStartTLSHandshakeAsServer performs a StartTLS Handshake (acting as server)
// It peeks into some bytes of conn and
...
// An error is returned if reading or writing to the WriteCloser fails.
